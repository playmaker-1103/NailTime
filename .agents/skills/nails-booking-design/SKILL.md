...
- Customers who want to book a nail appointment quickly.
- Salon owner/admin who needs to review bookings, follow up on WhatsApp, and manage services.
...
2. Review pending booking notices.
3. Follow up through WhatsApp.
4. Filter bookings by status.
5. Update booking status.
6. Add, edit, deactivate, or delete services.
...
Purpose: Let customers book immediately.
...
Keep the booking form visible and prominent. Do not add a marketing splash before the booking flow. Disable the time dropdown until a date is selected.

### Services Page

Route: `/services`

Purpose: Let customers browse available services and start booking from a chosen service.

Use simple, scan-friendly service cards with price, duration, description, and one clear booking action.

### Booking Success Page

Route: `/booking-success`

Confirm that the request was received. Show service name, appointment date/time, status, and actions to browse services or book another appointment when available.

### Admin Login

Route: `/admin/login`

Keep this minimal: email, password, sign in button, and error message. Do not expose setup instructions in the UI.

### Admin Dashboard

Route: `/admin`

Purpose: Manage bookings and services.

Include:

- Signed-in admin label
- Refresh and sign out actions
- Pending booking notice panel
- WhatsApp follow-up buttons
- Booking table
- Status filter
- Status dropdown per booking
- Delete booking action
- Add/edit service form
- Manage services list

This is operational UI, not a landing page. Prioritize readable tables, predictable controls, compact spacing, and icon buttons for repeated actions.
...
- Use system UI fonts.
- Keep headings bold and readable.
- Do not use viewport-scaled body text.
- Keep letter spacing at `0`.

Components:

- Buttons: 8px radius, icon + text for major actions.
- Icon buttons: square 40px buttons for table actions.
- Cards: 8px radius, only for individual service items, forms, panels, and repeated admin items.
- Forms: clear labels, strong focus state, validation message below the field.
- Tables: horizontal scroll on small screens.
- Pills: use for compact status or count labels.
...
- Vercel serverless entry is `api/index.js`.
- Supabase response mapping lives in `server/utils/dbMappers.js`.
- Preserve frontend-facing camelCase fields like `durationMinutes`, `isActive`, and `appointmentDate`.
- Use `VITE_API_URL=/api` for deployed builds.
...
# Embedded Design Taste Frontend Rules
...
# End Embedded Design Taste Frontend Rules
